...
class TestHelloGifCog(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_replies_with_gif_for_trigger_phrase(self):
        ...
        with patch("cogs.hello_gif.random.choice", return_value=RESPONSE_GIF_URL) as choice:
            await self.cog.on_message(message)

        choice.assert_called_once_with(RESPONSE_URLS)
        message.channel.send.assert_awaited_once_with(RESPONSE_GIF_URL)

    async def test_can_reply_with_image_for_trigger_phrase(self):
        message = _make_message(content="hi lol")

        with patch("cogs.hello_gif.random.choice", return_value=RESPONSE_IMAGE_URL) as choice:
            await self.cog.on_message(message)

        choice.assert_called_once_with(RESPONSE_URLS)
        message.channel.send.assert_awaited_once_with(RESPONSE_IMAGE_URL)
    ...
    async def test_ignores_messages_during_cooldown(self):
        ...
        with patch("cogs.hello_gif.random.choice", return_value=RESPONSE_GIF_URL):
